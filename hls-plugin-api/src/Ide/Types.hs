...
instance PluginMethod Request TextDocumentDocumentLink where
  pluginEnabled _ msgParams pluginDesc conf = pluginResponsible uri pluginDesc
      && pluginEnabledConfig plcDocumentLinkOn pid conf
    where
      uri = msgParams ^. J.textDocument . J.uri
      pid = pluginId pluginDesc
...
instance PluginRequestMethod TextDocumentDocumentLink where
  combineResponses _ _ _ _ x = sconcat x
